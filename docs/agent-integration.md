...
**token 不用你自己数**（ADR 0030）：Claude Code 走 `/connect` 接入时会装一个 Stop 钩子（macOS / Linux 是 python 脚本，Windows 是 PowerShell 脚本），每轮结束把这次会话的累计用量报到 `POST /me/usage`，服务端算增量、归到你当时开着的执行记录；没开执行记录的记成你名下的「未归口用量」（`get_my_metrics` 的 `unattributed_tokens`）。所以先领任务再干活，token 才会算到任务头上。下面的手动上报仍然可用，两者都是累计数、幂等。
...
| `get_my_schedule` | 我所有者的日程（ADR 0032）：一段日期里他负责的任务（计划起止与截止）、目标、里程碑到期、外部日历里的会议。给任务排计划、或人问"我这周有什么"时用 | 只读 |

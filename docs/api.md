...
| DELETE | `/org/teams/{id}` | 204。手工团队有成员、有下级也能删：成员离开它（只在这一个团队的人从此不属于任何团队）、直接下级上移到它的上级、归口到它的目标与迭代改成不归口，动态 `TeamDeleted` 记下各项数量。来自IM 集成的 →「来自飞书的团队不能删除，只能停用。」界面上删除前要先看影响范围并照着输入团队名二次确认 |
| GET | `/org/teams/{id}/impact` | 删除前的影响范围：`{members, only_team, sub_teams, goals, sprints, new_parent_id, new_parent, is_boundary}` |

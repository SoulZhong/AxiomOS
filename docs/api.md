...
| DELETE | `/org/teams/{id}` | 204。手工团队有成员、有下级也能删，**下级团队一起删**：子树里的成员离开这些团队（只在子树里有团队的人从此不属于任何团队）、归口到子树里任一团队的目标与迭代改成不归口，动态 `TeamDeleted` 记下各项数量。来自IM 集成的 →「来自飞书的团队不能删除，只能停用。」；子树里混着同步团队 →「下级团队「X」来自飞书，不能跟着一起删除；先把它挪走。」界面上删除前要先看影响范围并照着输入团队名二次确认 |
| GET | `/org/teams/{id}/impact` | 删除前的影响范围（按整棵子树算）：`{members, only_team, sub_teams, sub_team_names[], goals, sprints, is_boundary}` |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/schedule?from=&to=&who=me\|<member_id>\|<team_id>\|<org_id>` | 一段日期（含首尾，最长 62 天，默认本周）里若干人的日程 → `{from, to, who, members[{id,name}], items[], sources[]}`。`items` 每条 `{kind: task\|goal\|milestone\|event, id, title, member_id, member_name, start_date?, end_date?, date?, starts_at?, ends_at?, all_day?, number?, state?, progress?, deadline?, assignee_id?, status?, goal_id?, milestone_status?, provider?, url?, busy?, title_hidden?, overdue?}`：任务按计划起止（`start_date`/`end_date`）或单个日期（只有截止日时 `deadline: true`）；目标按计划起止；里程碑按到期日；会议带精确起止与全天。Agent 负责的任务算在它所有者的日程里。`who` 是团队时含下级团队的全部在职成员，再加上归口到这些团队的目标与其里程碑（负责人不在团队里也算，`member_name` 就是为这种情况带的），**不含外部日历的会议**（团队与整个组织的视图只看工作安排），可见范围照 ADR 0013（看不到的团队 / 成员 → 403）。`who` 是组织 ID 时是我能看的整个组织（界面上叫组织自己的名字）：可见域里全部团队的在职成员与归口的目标，能看全公司的人还包括不属于任何团队的人。**别人的会议只给「忙」**：`title_hidden: true`、没有标题与链接。`sources` 是接上的外部日历提供方 |
...
| PUT | `/me/calendars/{provider}` | 自助的提供方：`ics` 日历订阅链接 `{credentials: {url}}`；`caldav` CalDAV 账号 `{credentials: {server, username, password}}`（企业微信「同步至其他日历」给的那组、Apple iCloud 用 App 专用密码）→ 我的连接情况。链接 / 服务器先过出网守卫（只许 https，`webcal://` 自动转成 https，服务器不带协议时补 https），再拉一次证明能读（网页链接 →「这个链接返回的不是日历文件（.ics）。请到日历软件里取「订阅 / 私密地址」那条链接，不是网页地址。」；CalDAV 账号密码不对 →「账号或密码不对。企业微信的密码要现取，iCloud 要用 App 专用密码。」；回网页 →「这个地址不是 CalDAV 服务器…」；没有日历 →「这个账号下没有找到日历。」），整组加密存下、立刻同步一次。链接不回显；`self_service: true` 的提供方带 `fields / prerequisites / tip` 给个人设置页渲染，绑定行上带我自己的 `label / last_sync_at / last_status / last_status_title / last_error` |

...
- **甘特图的框可复用**（`gantt/GanttFrame.tsx` + `useGanttScale` + `useGanttShell` + `GanttControls`）：刻度、两层表头、今天线、缩放锚点、适应全部、全屏与"行是什么"分开；目标甘特图（§11）只需注入自己的行。

### 11. 目标甘特图与里程碑（ADR 0016）
目标页有两个页签：**树**（现状）与**甘特图**，`?view=tree|gantt`，同一批目标、同一个范围与筛选。

- **目标横条**：从计划开始到计划结束，进度按汇总百分比填充；截止日是横条外的一道短竖刻度；已放弃的目标灰色虚线；已达成的目标 success 色。子目标嵌套缩进，可折叠；折叠后父行显示汇总条。
- **里程碑 = 菱形**：画在所属目标的行上、对应日期处，8px；未到为 hairline 描边空心，已达到为 success 实心，逾期为 danger 实心并带小三角。悬停给出名称、日期、状态与"可以确认了"的提示；点击打开小气泡可确认或撤销。任务永远是横条，里程碑永远是菱形，两者不混用。
- **交互**：拖目标横条两端改计划起止；拖菱形改日期；在目标行的空白处双击新建里程碑（弹出名称输入，日期取双击处）。所有改动乐观更新，失败回滚并用后端的整句理由提示。
- **刻度与今天线**沿用任务甘特图（时 / 日 / 周 / 月 / 季 / 年、两层表头、缩放围绕光标、适应全部、全屏），实现上复用同一套刻度与表头组件。
- **任务甘特图按目标分组时**，目标汇总行同样画菱形，规则相同。
- **目标详情**新增「里程碑」面板：按日期列出，每条带状态、说明、「确认已达到 / 撤销」，末尾一行内联新增；目标行的状态摘要句在有逾期里程碑时优先提示「里程碑「X」已逾期」。
- 空状态：没有里程碑时甘特图行上不画任何东西，面板里一句「还没有里程碑，双击甘特图或在这里添加」。
...
- **左栏（团队树）**：顶部搜索框（按姓名、邮箱找人，命中后右栏定位到那个人所在团队并高亮）；树以组织名为根节点，每个团队一行：图标、名称、人数；共享边界的团队带一个小标记；来自 IM 同步的团队带「来自{IM 名}」提示且不能重命名。每行右侧 ⋮ 菜单：新建下级团队、重命名、移动到…、设为 / 取消共享边界、停用（有成员或下级时说明原因并禁用）。底部固定「＋ 新建团队」。可折叠展开，记住展开状态。

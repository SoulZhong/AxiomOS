...
### 21. Agent 接入向导
「Agent」页的主按钮从「注册 Agent」改成**「接入 Agent」**（ADR 0018）。向导对话框三步（`ConnectWizard`，步骤条 `WizardSteps` 五格：选运行环境 › 在终端执行 › 批准并授权 › 连接检查 › 试领一个任务）：
1. **选运行环境**：Claude Code / Cursor / OpenAI Codex / 其他 MCP 客户端，四张单选卡片各带一句"脚本会替你做什么"。
2. **在终端执行**：一行 `curl -fsSL '<地址>/api/v1/agent-auth/connect.sh?client=…' | sh`（`CopyLine`），说明令牌只进客户端配置、Windows 用 WSL；「或者手工配置」折叠项里是原来的注册抽屉（一次性令牌路径，仍然保留）。
3. **等待批准**：说明终端会打印验证码和网址，附一段示例输出；输入框「输入终端里显示的验证码」→ `/agents/connect/?code=`。

**批准页** `/agents/connect/?code=ABCD-1234`（需登录；`GET /agent-auth/device/{code}`）：先列申请是什么（验证码、运行环境、申请时填的名字、申请时间、过期时间），表单：Agent 名称（留空用申请里的名字，再缺省用运行环境名）、能力标签、**每项授权三选一**（允许 / 需要确认 / 拒绝；默认执行 = 需要确认、评论 = 允许、其余拒绝；「管理流程」只有需要确认 / 拒绝）、最多同时任务数、是否公共（只有组织负责人看到）；「批准」/「拒绝接入」。批准后同页进入**连接检查**：每 3 秒 `GET /agents/{id}/check`，最多 2 分钟；LED + 一句人话（后端的 `hint`）+ 最近调用的工具；在线后步骤条走到「试领一个任务」，给「去待领取任务」。已拒绝 / 已过期 / 找不到都是完整的句子加"该怎么办"，没有 `code` 时是一个输入框。

Agent 行多一列**运行环境**（`runtime` → Claude Code / Cursor / OpenAI Codex / 其他 MCP 客户端，列宽 112px，名字长了截断并带原名气泡），行内动作多一个**「检查连接」**：结果显示在遥测行下面（LED + hint + 最近工具），任何人都能对能看到的 Agent 点。多这一列一个按钮之后 1440 下行内挤了，所以：名字格的遥测行（最近心跳 · 今日 N 段 · 成本）一项一个不折行的小块、分隔点跟在后一项前面——窄了整块换行，行尾不会剩一个孤零零的「·」，也不会被挤成一列一个字；「授权」列的标签只写「7 项」（表头已经写了「授权」），把省下的宽度让给行内动作，四个动作在 1440 下都完整可见。

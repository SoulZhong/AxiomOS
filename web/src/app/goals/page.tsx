...
import { useAction, useHighlight, useLoad, useSprout } from "@/lib/hooks";
...
import { Avatar, Button, ConfirmDialog, Empty, ErrorBox, ListSkeleton, PageHeader, Panel, StatChips, Tag, Tip, cx } from "@/components/ui";

export default function GoalsPage() {
  const { session } = useSession();
  const goals = useLoad(() => api.goals.list(), []);
  // 目标行要说清"下一步做什么"，需要它名下任务的分布：一次拉全量任务在前端按目标树汇总
  const tasks = useLoad(() => api.tasks.list({ limit: 500 }), []);
  ...
  return (
    <div>
      <PageHeader title={t("goals.title")} description={t("goals.description")} actions={<Button variant="primary" icon={<IconPlus />} onClick={() => setCreating(null)}>{t("goals.new")}</Button>} />
      <StatChips
        className="mb-4"
        value={null}
        total={flat.length}
      ...
      />
      <Panel index={1} icon={<IconGoal />} title={t("nav.goals")} telemetry={goals.data ? t("panel.rows", { n: flat.length }) : undefined} padded={false}>
        {goals.loading ? (
          <ListSkeleton rows={5} />
        ) : goals.error ? (
          <div className="p-4"><ErrorBox message={goals.error} onRetry={goals.reload} /></div>
        ) : flat.length === 0 ? (
          <Empty text={t("goals.empty")} action={<Button variant="primary" icon={<IconPlus />} onClick={() => setCreating(null)}>{t("goals.new")}</Button>} />
        ) : (
          <ul>
            {(goals.data ?? []).map((g) => (
              <GoalRow key={g.id} goal={g} depth={0} collapsed={collapsed} onToggle={toggle} onAddChild={(id) => setCreating(id)} onAbandon={abandon} onDelete={setDeleting} onAchieve={achieve} tally={tally} busy={busy} currency={currency} highlight={highlight} />
            ))}
          </ul>
        )}
      </Panel>
      ...
      <GoalDrawer open={creating !== false} parentId={creating === false ? null : creating} goals={flat} onClose={() => setCreating(false)} onCreated={(g) => { mark(g.id); goals.reload(); }} />
    </div>
  );
}

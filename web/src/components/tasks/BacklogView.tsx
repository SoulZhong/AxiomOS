 ...
 */
export function BacklogView({ filters, ctx, reloadKey }: { filters: TaskFilters; ctx: MatchCtx; reloadKey: number }) {
  ...
  const shown = (items.data ?? []).filter((b) => matchTask({ goalId: b.task.goal_id, assigneeId: null, type: b.task.type, label: b.task.state.label, sprintId: b.task.sprint?.id ?? null }, filters, ctx, ["assignee", "team"]));
  ...
  return (
    <div>
      ...
      <Panel index={1} icon={<IconBacklog />} title={t("backlog.title")} telemetry={items.data ? t("panel.rows", { n: shown.length }) : undefined} padded={false}>
        {items.loading && !items.data ? <TableSkeleton rows={4} cols={7} /> : items.error ? <div className="p-4"><ErrorBox message={items.error} onRetry={items.reload} /></div> : shown.length === 0 ? <Empty text={hasFilters(filters) ? t("tasks.noMatch") : t("backlog.empty")} /> : (
          <Table>
            <thead>
              <tr>

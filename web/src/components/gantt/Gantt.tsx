...
import { memo, useCallback, useEffect, useImperativeHandle, useLayoutEffect, useMemo, useRef, useState, type KeyboardEvent as ReactKeyboardEvent, type PointerEvent as ReactPointerEvent, type Ref, type RefObject, type TransitionEvent as ReactTransitionEvent } from "react";
...
import { buildHeader, DAY_MS, isMacro, xOfTime, type Cell, type Scale } from "./scales";

/*
 * 甘特图 v2（DESIGN.md §8「宏观到微观」+ §Axiom 9「航线图」）。
 * 结构：一个滚动容器（两个方向都在内部滚），里面是
 *   - 固定在顶部的两层表头（左上角同时固定在左侧），
 *   - 每个分组一行"组行"，下面一个 .fold 折叠区装任务行（200ms / 150ms 高度过渡，沿用目标树），
 *   - 每一行 = 固定在左侧的标签格 + 一张与图区同宽的小 SVG（条、执行记录段、汇总条），
 *   - 铺在底下的网格层（竖线、周末底纹、今天线）与盖在上面的前置箭头层。
...
const HEADER_H = 56;
const RULER_Y = 28;
const ROW_H: Record<Density, number> = { compact: 28, comfortable: 36 };
const BAR_H: Record<Density, number> = { compact: 12, comfortable: 16 };
const ACTUAL_H = 4;
...
const alpha = (token: string, pct: number) => `color-mix(in srgb, var(--c-${token}) ${pct}%, transparent)`;
const C = {
  accent: v("accent"),
...
const MONO = "var(--font-jetbrains), 'SF Mono', Menlo, monospace";
...
export const STATE_FILL: Record<StateLabel, { bg: string; border: string; text: string }> = {
  ...
  waiting: { bg: alpha("warning", 18), border: alpha("warning", 45), text: v("warning") },
  terminal_success: { bg: alpha("success", 18), border: alpha("success", 45), text: v("success") },
  terminal_failure: { bg: alpha("neutral", 10), border: alpha("neutral", 25), text: C.subtle },
};
/** 状态点（窄列 / 行首）：与状态徽标同一套语义色 */
const STATE_DOT: Record<StateLabel, string> = { pending: v("neutral"), active: C.accent, waiting: v("warning"), terminal_success: v("success"), terminal_failure: v("neutral") };
...
const textW = (s: string, size = 11) => {
  let w = 0;
  for (const ch of s) w += /[⺀-鿿豈-﫿＀-￯]/.test(ch) ? size : size * 0.6;
  return w;
};
function clipToWidth(s: string, maxPx: number, size = 11): string {
  if (textW(s, size) <= maxPx) return s;
  let out = "";
  let w = 0;
  for (const ch of s) {
    const cw = /[⺀-鿿豈-﫿＀-￯]/.test(ch) ? size : size * 0.6;
...
export function Gantt({ ref, data, groups, from, days, dayW, scale, onZoom, showCost, showOverdue, onCreate, runs, collapsed, onToggle, density, labelW: labelWProp, narrow, onLabelW, onNarrow, scrollRef }: Props) {
  ...
  const labelW = narrow ? LABEL_NARROW : labelWProp;
  ...
  const win = virtual ? [Math.max(0, Math.floor((view.top - OVERSCAN) / rowH)), Math.ceil((view.top + view.h + OVERSCAN) / rowH)] : [0, totalRows];
  ...
  // 切档 / 换一批数据时整张图会重排：用一次 200ms 的"0.6 透明 + 2px 模糊 → 清晰"把硬切换糊成一次平滑的重绘（WAAPI，不重挂、不丢滚动位置）。
  // 首次渲染不播；连续缩放（dayW 变但档位没变）不播；reduced-motion 下只做透明度。
  ...
  useEffect(() => {
    ...
    const anim = el.animate(frames, { duration: 200, easing: easeOut() });
    return () => anim.cancel();
  }, [scale, data]);
  ...
  return (
    <div ref={panelRef} className="gantt-panel">
      <div ref={scrollRef} className={`gantt-scroll gantt-drag ${onCreate ? "gantt-creatable" : ""}`} data-density={density}>
        {/* 表头：固定在顶部；左上角同时固定在左侧 */}
        ...
        <div className="gantt-body" style={{ width: totalW, minHeight: bodyH }}>
          ...
          {groups.map((g, gi) => {
            ...
            return (
              <div key={g.key} className="gantt-group">
                {groupVisible ? (
                  <GroupRow g={g} open={open} macro={macro} summary={s} rowH={rowH} barH={barH} width={width} labelW={labelW} narrow={narrow} dayW={dayW} cells={header.bottom} xOf={xOf} onToggle={onToggle} drag={drag?.rowKey === g.key ? drag : null} onDown={onDown} onMove={onMove} onUp={onUp} creatable={!!onCreate} />
                ) : (
          ...
          {/* 前置箭头 */}
          <svg className="gantt-deps" style={{ left: labelW, width, opacity: folding ? 0 : 1 }} aria-hidden="true">
            <defs>

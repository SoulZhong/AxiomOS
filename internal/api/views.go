...
// names 是 ID → 名字（动态句子渲染在 app 层，只要名字）。
func (r refs) names() map[string]string {
...
func enumTitle(loc i18n.Locale, kind, v string) string { return app.EnumTitle(loc, kind, v) }
...
func textOf(v any, loc i18n.Locale) string { return app.TextOf(v, loc) }

// eventSummary 把一条动态渲染成一句话（实现在 app.EventSummary）。
func eventSummary(e *store.EventRow, r refs, taskTitle map[string]string, roles map[string]i18n.Text, loc i18n.Locale) string {
	return app.EventSummary(e, r.names(), taskTitle, roles, loc)
}

func approvalNote(e *store.EventRow, r refs, loc i18n.Locale) string {
	return app.ApprovalNote(e, r.names(), loc)
}

func numOf(v any) (int, bool) { return app.NumOf(v) }
...
func proposalSummary(sum string) string { return app.ProposalSummaryText(sum) }
